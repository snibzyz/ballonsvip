        ...
        if shift_pressed:
            if num_sel == 0:
                ...
                if ctrl_pressed:
                    ...
                    # Use sel_anchor_widget if available, otherwise fall back to first selected item
                    anchor_idx = self.sel_anchor_widget.idx if self.sel_anchor_widget is not None else old_idx_list[0]
                    sel_min, sel_max = min(anchor_idx, tgt_w.idx), max(anchor_idx, tgt_w.idx)
